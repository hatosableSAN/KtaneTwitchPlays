...
public class UnsupportedModComponentSolver : ComponentSolver
{
		...
		: base(module, componentSolverFields == null || componentSolverFields.HookUpEvents)
	{
		...
		ModInfo = new ModuleInformation { scoreString = "0", builtIntoTwitchPlays = true, helpText = $"!{{0}} solve[{(_bombModule != null ? "モジュール" : "特殊モジュール")} を解除する]", moduleDisplayName = $"Unsupported Twitchplays Module  ({module.BombComponent.GetModuleDisplayName()})", moduleID = "UnsupportedTwitchPlaysModule" };
